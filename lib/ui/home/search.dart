...
import 'package:devkitflutter/model/last_seen_model.dart';
import 'package:devkitflutter/model/search_model.dart';
  ...
  Widget _buildLastSeenCard(index, boxImageSize){
    return Container(
      ...
      child: GestureDetector(
        behavior: HitTestBehavior.translucent,
        onTap: () {
          Navigator.push(context, MaterialPageRoute(builder: (context) => ProductDetailPage(name: lastSeenData[index].name, image: lastSeenData[index].image, price: lastSeenData[index].price, rating: lastSeenData[index].rating, review: lastSeenData[index].review, sale: lastSeenData[index].sale,color:[],size:[])));
        },
        child: ClipRRect(
            borderRadius: BorderRadius.all(
              Radius.circular(10),
